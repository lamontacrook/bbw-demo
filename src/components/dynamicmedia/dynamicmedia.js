...
const DynamicMedia = ({ asset, alt = 'WKND image', title = 'Title Missing', itemProp = 'asset', rendition }) => {
  const context = useContext(AppContext);
  const [imageFile, setImageFile] = useState('');
  const [domain, setDomain] = useState('');

  useEffect(() => {
    const src = context.serviceURL.includes('publish-') ? 
      asset?._authorUrl.replace('author', 'publish') : 
      asset?._authorUrl;
  ...
  return (
    <picture>
      <img src={`${domain}is/image/${imageFile}?$${rendition}$`} title={title} alt={alt} data-aue-prop={itemProp} data-aue-type="media" data-aue-label='Asset' />
